...
export function SiteMapSection({ campgroundId, campgroundName }: SiteMapSectionProps) {
  const insights = getInsightsForCampground(campgroundId);
  const hasMap = !!getSiteMap(campgroundId);
  ...
  const hasLists =
    !!insights && (insights.recommendedSites.length > 0 || insights.avoidSites.length > 0);
  if (!hasMap && !hasLists) return null;
  ...
  return (
    <section>
      ...
      {/* Numbered site map: the actual per-site map, used together with the lists below */}
      {hasMap && (
        <NumberedSiteMap bare campgroundId={campgroundId} campgroundName={campgroundName} />
      )}
